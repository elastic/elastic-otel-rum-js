...
const DATA_DIR = path.resolve(import.meta.dirname, 'android-retrace-data');
...
export const androidMapFixtures: AndroidMapFixture[] = scenarios.map(
    ({name, documents}) => ({
        name,
        classMaps: documents.map((fileName) => ({
            content: fs
                .readFileSync(path.resolve(DATA_DIR, 'es-documents', fileName))
                .toString(),
            fileName,
        })),
        expected: fs
