...
`;
  ...
  var Logger = class {
    ...
    emit(logRecord) {
      ...
      const currentContext2 = logRecord.context || context.active();
      ...
      if (loggerConfig.traceBased) {
        const spanContext = trace.getSpanContext(currentContext2);
        if (spanContext && isSpanContextValid(spanContext)) {
      ...
      this._sharedState.activeProcessor.onEmit(logRecordInstance, currentContext2);
  ...
  var ExceptionInstrumentation = class extends InstrumentationBase {
    ...
    onError(event) {
      ...
      const logger3 = logs.getLoggerProvider().getLogger(this.instrumentationName, this.instrumentationVersion);
  ...
  // lib/context.js
  var logger2 = createLogger({ logLevel: "warn" });
  var currentContext = ROOT_CONTEXT;
  var AsyncApisContextManager = {
    active: function() {
      return currentContext;
    },
    with: function(context2, fn, thisArg, ...args) {
      const prevContext = currentContext;
      currentContext = context2 || ROOT_CONTEXT;
      try {
        return fn.call(thisArg, ...args);
      } finally {
    ...
    bind: function(context2, target) {
      if (typeof target === "function") {
        return bindFn(target, this, context2);
      }
      return target;
    },
    enable: function() {
      const manager = this;
      wrap2(window, "setTimeout", (origSetTimeout) => {
      ...
      wrap2(window, "setImmediate", (origSetImmediate) => {
        return function(...args) {
          args[0] = bindFn(args[0], manager, manager.active());
    ...
    disable: function() {
      unwrap2(window, "setTimeout");
      unwrap2(window, "setImmediate");
  ...
  function bindFn(fn, manager, context2) {
    if (typeof fn !== "function") {
      return fn;
    }
    const ctx = context2 || manager.active();
  ...
  function wrapPromise(manager) {
    wrap2(Promise.prototype, "then", (origThen) => {
    ...
    wrap2(Promise.prototype, "catch", (origCatch) => {
    ...
    wrap2(Promise.prototype, "finally", (origFinally) => {
  ...
  function unwrapPromise() {
    unwrap2(Promise.prototype, "then");
    unwrap2(Promise.prototype, "catch");
    unwrap2(Promise.prototype, "finally");
  }
  var xhrProps = ["onreadystatechange"];
  ...
  var xhrTargetProto = globalThis.XMLHttpRequestEventTarget.prototype;
  function wrapXMLHttpRequest(manager) {
    wrap2(xhrProto, "addEventListener", function(origAEL) {
      return function(...args) {
        const xhr = this;
        if (typeof args[1] === "function") {
          xhr.__bound = xhr.__bound || /* @__PURE__ */ new Map();
    ...
    wrap2(xhrProto, "removeEventListener", function(origREL) {
      return function(...args) {
        const xhr = this;
        if (typeof args[1] === "function") {
          const handler = args[1];
  ...
  function unwrapXMLHttpRequest() {
    unwrap2(xhrProto, "addEventListener");
    unwrap2(xhrProto, "removeEventListener");
    for (const prop of xhrProps) {
      const descriptor = Object.getOwnPropertyDescriptor(xhrProto, prop);
      if (descriptor) {
        unwrap2(descriptor, "set");
        unwrap2(descriptor, "get");
        Object.defineProperty(xhrProto, prop, descriptor);
      }
    }
    for (const prop of xhrTargetProps) {
  ...
  function wrap2(nodule, name, wrapper) {
    if (!nodule || !nodule[name]) {
      logger2.warn("no original function " + String(name) + " to wrap");
      return;
    }
    if (!wrapper) {
      logger2.warn("no wrapper function");
      logger2.warn(new Error().stack);
      return;
    }
    const original = nodule[name];
    if (typeof original !== "function" || typeof wrapper !== "function") {
      logger2.warn("original object and wrapper must be functions");
      return;
    }
    const wrapped = wrapper(original, name);
  ...
  function unwrap2(nodule, name) {
    if (!nodule || !nodule[name]) {
      logger2.warn("no function to unwrap.");
      logger2.warn(new Error().stack);
      return;
    }
    const wrapped = nodule[name];
  ...
  function wrapDescriptor(descriptor, manager) {
    wrap2(descriptor, "set", function(origSet) {
    ...
    wrap2(descriptor, "get", function(origGet) {
      return function() {
  ...
  // lib/detector.js
  function detectResource(attribs, serviceName2, serviceVersion) {
    if (typeof serviceName2 === "string" && serviceName2) {
      attribs["service.name"] = serviceName2;
    }
  ...
  function startBrowserSdk(cfg = {}) {
    ...
    const { serviceName: serviceName2, serviceVersion } = cfg;
    ...
    const instrFactories = {
      "document-load": (cfg2) => new DocumentLoadInstrumentation(cfg2),
      fetch: (cfg2) => new FetchInstrumentation(cfg2),
      "long-task": (cfg2) => new LongTaskInstrumentation(cfg2),
      "user-interaction": (cfg2) => new UserInteractionInstrumentation(cfg2),
      "xml-http-request": (cfg2) => new XMLHttpRequestInstrumentation(cfg2),
      "web-exception": (cfg2) => new ExceptionInstrumentation(cfg2)
    };
